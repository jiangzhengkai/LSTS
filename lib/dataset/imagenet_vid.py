...
class ImageNetVID(IMDB):
    ...
    def load_image_set_index(self):
        ...
        if len(lines[0]) == 2:
            ...
            self.video_name = [x[0] for x in lines]
            self.frame_id = [int(x[1]) for x in lines]
    ...
    def evaluate_detections_multiprocess_seqnms(self, detections, gpu_id):
        ...
        result_dir = os.path.join(self.result_path, 'seq_nms_results')
        if not os.path.exists(result_dir):
            os.mkdir(result_dir)
    ...
    def get_result_file_template_seqnms(self, gpu_id):
        ...
        :return: a string template
        """
        res_file_folder = os.path.join(self.result_path, 'seqnmsresults')
    ...
    def write_vid_results_multiprocess_seqnms(self, detection, gpu_id):
        ...
        print 'Writing Seq NMS {} ImageNetVID results file'.format('all')
    ...
    def do_python_eval_gen(self, gpu_number=None):
        ...
        # filename = self.get_result_file_template().format('all')
        ap = vid_eval(filenames, annopath, imageset_file, self.classes_map, annocache, ovthresh=0.5,
