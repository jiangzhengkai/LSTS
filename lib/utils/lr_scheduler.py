...
class WarmupMultiFactorScheduler(LRScheduler):
    ...
    """
    def __init__(self, step, factor=1, warmup=False, warmup_lr=0,warmup_end_lr=0, warmup_step=0, linear=False):
    ...
    def __call__(self, num_update):
        ...
        if self.warmup and num_update < self.warmup_step:
           if self.warmup_linear:
                self.base_lr = self.warmup_lr + (self.warmup_end_lr - self.warmup_lr)/self.warmup_step * num_update
                if num_update % 100 == 0 and num_update != self.old_num_update:
                    logging.info("Update[%d]: Warmup learning rate %0.5e", num_update, self.base_lr)
                    self.old_num_update = num_update
                return self.base_lr
           else:
               if num_update % 100 == 0 and num_update != self.old_num_update:
                    logging.info("Update[%d]: Warmup learning rate %0.5e,%0.5e", num_update, self.warmup_lr,self.base_lr)
                    self.old_num_update = num_update
