...
def im_detect_impression_online(predictor, data_batch, data_names, scales, cfg):
    output_all = predictor.predict(data_batch)
    ...
    for output, data_dict, scale in zip(output_all, data_dict_all, scales):
        if cfg.TEST.HAS_RPN:
            ...
            rois = data_dict['rois'].asnumpy().reshape((-1, 5))[:, 1:]
        im_shape = data_dict['data_cur'].shape
    ...
    if output_all[0].has_key('feat_conv_3x3_relu_output'):
        conv_feat = output_all[0]['feat_conv_3x3_relu_output']
    else:
        conv_feat = None

    if output_all[0].has_key('feat_task_output'):
        feat_task = output_all[0]['feat_task_output']
    else:
        feat_task = None

    if output_all[0].has_key('impression_update_output'):
        impression = output_all[0]['impression_update_output']
    else:
        impression = None
    if output_all[0].has_key('feat_task_cur_output'):
        feat_task_cur = output_all[0]['feat_task_cur_output']
    else:
        feat_task_cur = None

    return scores_all, pred_boxes_all, data_dict_all, conv_feat, impression, feat_task ,feat_task_cur


def im_detect_impression_offline(predictor, data_batch, data_names, scales, cfg):
    output_all = predictor.predict(data_batch)
    ...
    return scores_all, pred_boxes_all, data_dict_all, conv_feat, impression, feat_task
...
def pred_eval_impression_online(gpu_id, first_predictor, cur_predictor, key_predictor, test_data, imdb, cfg, vis=False, thresh=1e-4, logger=None, ignore_cache=True):
    """
    wrapper for calculating offline validation for faster data analysis
    in this example, all threshold are set by hand
    :param predictor: Predictor
    ...
    for im_info, key_frame_flag, data_batch in test_data:
        ...
        t= time.time()
        scales = [iim_info[0, 2] for iim_info in im_info]
        if key_frame_flag != 2: # keyframe
            if key_frame_flag == 0: # first frame
                scores_all, pred_boxes_all, data_dict_all, conv_feat, _, _ = im_detect_impression_online(first_predictor, data_batch, data_names, scales, cfg)
                feat_task = conv_feat
                ...
                scores_all, pred_boxes_all, data_dict_all, _, impression, feat_task = im_detect_impression_online(key_predictor, data_batch, data_names, scales, cfg)
                i = i + 1
        else: # current frame
            data_batch.data[0][-1] = feat_task
            data_batch.provide_data[0][-1] = ('key_feat_task', feat_task.shape)
            scores_all, pred_boxes_all, data_dict_all, _, _, _, = im_detect_impression_online(cur_predictor, data_batch, data_names, scales, cfg)
            i = i + 1
        if key_frame_flag == 0:
            roidb_idx += 1
            roidb_offset = 0
...
def pred_eval_multiprocess_impression_online(gpu_num, first_predictors, cur_predictors, key_predictors, test_datas, imdb, cfg, vis=False, thresh=1e-4, logger=None, ignore_cache=True):
    if gpu_num == 1:
        res = [pred_eval_impression_online(0, first_predictors[0], cur_predictors[0], key_predictors[0], test_datas[0], imdb, cfg, vis, thresh, logger, ignore_cache),]
    else:
        from multiprocessing.pool import ThreadPool as Pool
        pool = Pool(processes=gpu_num)
        multiple_results = [pool.apply_async(pred_eval_impression_online,args=(i, first_predictors[i], cur_predictors[i],key_predictors[i], test_datas[i], imdb, cfg, vis, thresh, logger, ignore_cache)) for i in range(gpu_num)]
        pool.close()
        pool.join()
        res = [res.get() for res in multiple_results]
    info_str = imdb.evaluate_detections_multiprocess(res)
...
def pred_eval_impression_offline(gpu_id, first_predictor, cur_predictor, key_predictor, test_data, imdb, cfg, vis=False, thresh=1e-4, logger=None, ignore_cache=True):
    """
    wrapper for calculating offline validation for faster data analysis
    in this example, all threshold are set by hand
    :param predictor: Predictor
    ...
    for im_info, key_frame_flag, data_batch in test_data:
        ...
        for delta, (scores, boxes, data_dict) in enumerate(zip(scores_all, pred_boxes_all, data_dict_all)):
            ...
            if max_per_image > 0:
                image_scores = np.hstack([all_boxes[j][idx + delta][:, -1] for j in range(1, imdb.num_classes)])
...
def pred_eval_seq_nms(gpu_id, imdb, res):
    imdb.evaluate_detections_multiprocess_seqnms(res, gpu_id)

def pred_eval_multiprocess_impression_offline(gpu_num, first_predictors, cur_predictors, key_predictors, test_datas, imdb, cfg, vis=False, thresh=1e-4, logger=None, ignore_cache=True):
    if cfg.TEST.SEQ_NMS == False:
        if gpu_num == 1:
            res = [pred_eval_impression_offline(0, first_predictors[0], cur_predictors[0], key_predictors[0], test_datas[0], imdb, cfg, vis, thresh, logger, ignore_cache),]
        else:
            from multiprocessing.pool import ThreadPool as Pool
            pool = Pool(processes=gpu_num)
            multiple_results = [pool.apply_async(pred_eval_impression_offline,args=(i, first_predictors[i], cur_predictors[i],key_predictors[i], test_datas[i], imdb, cfg, vis, thresh, logger, ignore_cache)) for i in range(gpu_num)]
            pool.close()
            pool.join()
            res = [res.get() for res in multiple_results]
        info_str = imdb.evaluate_detections_multiprocess(res)
        ...
        if gpu_num == 1:
            res = [pred_eval_impression_offline_seq_nms(0, first_predictors[0], cur_predictors[0], key_predictors[0], test_datas[0], imdb, cfg, vis, thresh, logger, ignore_cache),]
        else:
            from multiprocessing.pool import ThreadPool as Pool
            pool = Pool(processes=gpu_num)
            multiple_results = [pool.apply_async(pred_eval_impression_offline_seq_nms,args=(i, first_predictors[i], cur_predictors[i],key_predictors[i], test_datas[i], imdb, cfg, vis, thresh, logger, ignore_cache)) for i in range(gpu_num)]
            pool.close()
            pool.join()
            res = [res.get() for res in multiple_results]
        ...
        for i in range(gpu_num):
            job = pool.apply_async(pred_eval_seq_nms, args=(i, imdb, res[i]))
            jobs.append(job)
            #job = apply_async(pool, pred_eval_seq_nms, (i, imdb, res[i]))
            #jobs.append(job)
        #pool.close()
        #pool.join()
        for job in jobs:
